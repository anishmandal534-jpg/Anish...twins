...
This version uses OpenRouter's OpenAI-compatible API at `https://openrouter.ai/api/v1`.
Keep the API key only in `backend/.env`, never in frontend JavaScript or Git.
...
In Settings, keep Backend URL as:
`http://localhost:8000`
